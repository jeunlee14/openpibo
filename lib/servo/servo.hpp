...
int
get_motor_profile(int o, int p, int r)
{
  FILE *fp;
  ...
  if(p && (fp = fopen(POS_PATH, "r")))
  {
    ...
    fclose(fp);
    memcpy(motor_pos_tmp, motor_pos, 10*sizeof(int));
  }

  if(r && (fp = fopen(RECORD_PATH, "r")))
  {
    fscanf(fp, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
      &motor_rec[0], &motor_rec[1], &motor_rec[2], &motor_rec[3], &motor_rec[4],
      &motor_rec[5], &motor_rec[6], &motor_rec[7], &motor_rec[8], &motor_rec[9]);
    fclose(fp);
  }
...
int
set_motor_profile(int o, int p, int r)
{
  FILE * fp;
  ...
  if(r && (fp = fopen(RECORD_PATH, "w")))
  {
    fprintf(fp, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
      abs(motor_pos[0] - motor_pos_tmp[0])/10 + motor_rec[0],
      abs(motor_pos[1] - motor_pos_tmp[1])/10 + motor_rec[1],
      abs(motor_pos[2] - motor_pos_tmp[2])/10 + motor_rec[2],
      abs(motor_pos[3] - motor_pos_tmp[3])/10 + motor_rec[3],
      abs(motor_pos[4] - motor_pos_tmp[4])/10 + motor_rec[4],
      abs(motor_pos[5] - motor_pos_tmp[5])/10 + motor_rec[5],
      abs(motor_pos[6] - motor_pos_tmp[6])/10 + motor_rec[6],
      abs(motor_pos[7] - motor_pos_tmp[7])/10 + motor_rec[7],
      abs(motor_pos[8] - motor_pos_tmp[8])/10 + motor_rec[8],
      abs(motor_pos[9] - motor_pos_tmp[9])/10 + motor_rec[9]
      );
    fclose(fp);
  }
...
int SetMultiTarget(int* pTarget)
{
  ...
  for(int ch =0; ch < 10; ch++){
    motor_pos[ch] = pTarget[ch]==NS?motor_pos[ch]:pTarget[ch];
  }
#ifdef DBG
  printf("Entering the setMultiTarget\n");
#endif
  int idx = 0;
  unsigned char command[23] = {0,};
